...
struct ShaderParams {
    // Colors
    color_petal_start_a: [f32; 3],  // Left side start color
    _pad1: f32,
    color_petal_end_a: [f32; 3],    // Left side end color
    _pad2: f32,
    color_petal_start_b: [f32; 3],  // Right side start color
    _pad3: f32,
    color_petal_end_b: [f32; 3],    // Right side end color
    _pad4: f32,
    bg_color: [f32; 3],             // Background color
    _pad5: f32,
...
impl ShaderManager for Shader {
    fn init(core: &Core) -> Self {
        ...
        let params_uniform = UniformBinding::new(
            ...
            "Params Uniform",
            ShaderParams {
                // Colors with original yellow-black gradients
                color_petal_start_a: [1.0, 1.0, 0.0],  // yellow
                _pad1: 0.0,
                color_petal_end_a: [0.0, 0.0, 0.0],    // black
                _pad2: 0.0,
                color_petal_start_b: [0.0, 0.0, 0.0],  // black
                _pad3: 0.0,
                color_petal_end_b: [1.0, 1.0, 0.0],    // yellow
                _pad4: 0.0,
        ...
        let fs_module = core.device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(include_str!("../../shaders/asahi.wgsl").into()),
        });

        let shader_paths = vec![
            PathBuf::from("shaders/vertex.wgsl"),
            PathBuf::from("shaders/asahi.wgsl"),
        ];

        let base = BaseShader::new(
            core,
            include_str!("../../shaders/vertex.wgsl"),
            include_str!("../../shaders/asahi.wgsl"),
            &bind_group_layouts,
            None,
        );
            ...
            self.base.render_ui(core, |ctx| {
                ...
                egui::Window::new("Flower Settings").show(ctx, |ui| {
                    ui.collapsing("Left Petals Colors", |ui| {
                        ...
                        changed |= ui.color_edit_button_rgb(&mut params.color_petal_end_a).changed();
                        ui.label("End Color");
                    });
        
                    ui.collapsing("Right Petals Colors", |ui| {
                        changed |= ui.color_edit_button_rgb(&mut params.color_petal_start_b).changed();
                        ui.label("Start Color");
                        
                        changed |= ui.color_edit_button_rgb(&mut params.color_petal_end_b).changed();
                        ui.label("End Color");
                    });
                    ...
                    ui.collapsing("Additional Settings", |ui| {
                        ...
                        changed |= ui.add(egui::Slider::new(&mut params.animation_speed, 0.0..=12.0)
