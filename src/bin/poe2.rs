    ...
    fn init(core: &Core) -> Self {
        ...
        let params_uniform = UniformBinding::new(
            ...
            ShaderParams {
                ...
                _pad1: 0.0,
                hi_color: [1.0, 0.95, 0.8],
                _pad2: 0.0,
                sh_color: [0.2, 0.22, 0.25],
                _pad3: 0.0,
                
                r_base: [0.62, 0.25, 0.13],
                _pad4: 0.0,
                r_dark: [0.35, 0.15, 0.08],
                _pad5: 0.0,
                m_spec: [0.9, 0.8, 0.7],
                _pad6: 0.0,
        ...
        let fs_module = core.device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(include_str!("../../shaders/poe2.wgsl").into()),
        });

        let shader_paths = vec![
            PathBuf::from("shaders/vertex.wgsl"),
            PathBuf::from("shaders/poe2.wgsl"),
        ];

        let base = BaseShader::new(
            core,
            include_str!("../../shaders/vertex.wgsl"),
            include_str!("../../shaders/poe2.wgsl"),
            &bind_group_layouts,
            None,
        );
            ...
            self.base.render_ui(core, |ctx| {
                ctx.style_mut(|style| {
                    ...
                    ui.collapsing("Colors", |ui| {
                        ...
                        changed |= ui.color_edit_button_rgb(&mut params.bg_color).changed();
                        ui.label("Background Color");
                        
                        changed |= ui.color_edit_button_rgb(&mut params.hi_color).changed();
                        ui.label("Highlight Color");
                        
                        changed |= ui.color_edit_button_rgb(&mut params.sh_color).changed();
                        ui.label("Shadow Color");
                        
                        ui.separator();
                        ui.label("Metal Colors");
                        changed |= ui.color_edit_button_rgb(&mut params.r_base).changed();
                        ui.label("Base Rust Color");
                        
                        changed |= ui.color_edit_button_rgb(&mut params.r_dark).changed();
                        ui.label("Dark Rust Color");
                        
                        changed |= ui.color_edit_button_rgb(&mut params.m_spec).changed();
                        ui.label("Metal Specular");
                    });
        
                    ui.collapsing("Light & Effects", |ui| {
                        changed |= ui.add(egui::Slider::new(&mut params.light_pos[0], -5.0..=5.0)
                            .text("Light X")).changed();
                        changed |= ui.add(egui::Slider::new(&mut params.light_pos[1], -5.0..=5.0)
                            .text("Light Y")).changed();
                        
                        changed |= ui.add(egui::Slider::new(&mut params.ao_strength, 0.0..=1.0)
                            .text("AO Strength")).changed();
                        
                        changed |= ui.add(egui::Slider::new(&mut params.noise_scale, 0.0..=3.0)
                            .text("Noise Scale")).changed();
                    });
        
                    ui.collapsing("Wear & Animation", |ui| {
                        changed |= ui.add(egui::Slider::new(&mut params.wear_amount, 0.0..=0.05)
                            .text("Wear Amount")).changed();
                        
                        changed |= ui.add(egui::Slider::new(&mut params.wear_scale, 50.0..=300.0)
                            .text("Wear Scale")).changed();
                        
                        changed |= ui.add(egui::Slider::new(&mut params.speed, 0.0..=2.0)
