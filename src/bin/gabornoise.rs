                ...
                ctx.style_mut(|style| {
                    ...
                    .show(ctx, |ui| {
                        ...
                        ui.collapsing("Resolution", |ui| {
                            changed |= ui.add(
                                egui::Slider::new(&mut params.width, 10.0..=640.0)
                                    .text("Width")
                            ).changed();
                            changed |= ui.add(
                                egui::Slider::new(&mut params.height, 10.0..=480.0)
                                    .text("Height")
                            ).changed();
                            changed |= ui.add(
                                egui::Slider::new(&mut params.steps, 2.0..=24.0)
                                    .text("Steps")
                            ).changed();
                        });
                        ui.collapsing("Gabor Noise", |ui| {
                            changed |= ui.add(
                                egui::Slider::new(&mut params.kernel_size, 0.1..=10.0)
                                    .text("Kernel Size")
                            ).changed();
                            changed |= ui.add(
                                egui::Slider::new(&mut params.num_kernels, 1.0..=100.0)
                                    .text("Num Kernels")
                            ).changed();
                            changed |= ui.add(
                                egui::Slider::new(&mut params.frequency, 1.0..=100.0)
                                    .text("Frequency")
                            ).changed();
                            changed |= ui.add(
                                egui::Slider::new(&mut params.frequency_var, 0.0..=100.0)
                                    .text("Frequency Variation")
                            ).changed();
                        });
                        ui.collapsing("Animation", |ui| {
                            changed |= ui.add(
                                egui::Slider::new(&mut params.seed, 1.0..=99999.0)
                                    .text("Seed")
                            ).changed();
                            changed |= ui.add(
                                egui::Slider::new(&mut params.animation_speed, 0.0..=5.0)
                                    .text("Animation Speed")
                            ).changed();
                            changed |= ui.add(
                                egui::Slider::new(&mut params.gamma, 0.3..=1.5)
                                    .text("Gamma")
                            ).changed();
                        });
