        ...
        let full_output = if self.base.key_handler.show_ui {
            self.base.render_ui(core, |ctx| {
                ctx.style_mut(|style| {
                    ...
                    changed |= ui.add(egui::Slider::new(&mut params.sigma, 0.0..=12.0).text("star")).changed();
                    changed |= ui.add(egui::Slider::new(&mut params.gamma, 0.0..=3.0).text("light_r")).changed();
                    changed |= ui.add(egui::Slider::new(&mut params.blue, 0.0..=3.0).text("light_b")).changed();
                    changed |= ui.add(egui::Slider::new(&mut params.a, 0.0..=3.0).text("light_g")).changed();
                    changed |= ui.add(egui::Slider::new(&mut params.b, 0.0..=0.2).text("bgr")).changed();
                    changed |= ui.add(egui::Slider::new(&mut params.c, 0.0..=0.2).text("bgb")).changed();
                    changed |= ui.add(egui::Slider::new(&mut params.d, 0.0..=0.2).text("bgg")).changed();
                    changed |= ui.add(egui::Slider::new(&mut params.g, 0.0..=20.0).text("light1")).changed();
