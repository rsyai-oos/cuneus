...
impl UniformProvider for QuadraticBulbParams {
    fn as_bytes(&self) -> &[u8] {
...
struct QuadraticBulbShader {
    base: RenderKit,
    params_uniform: UniformBinding<QuadraticBulbParams>,
...
impl QuadraticBulbShader {
    fn recreate_compute_resources(&mut self, core: &Core) {
        ...
        self.compute_bind_group = core.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("QuadraticBulb Compute Bind Group"),
...
impl ShaderManager for QuadraticBulbShader {
    fn init(core: &Core) -> Self {
        ...
        let compute_bind_group_layout = core.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            label: Some("quadraticbulb_compute_output_layout"),
        });
        ...
        let params_uniform = UniformBinding::new(
            ...
            QuadraticBulbParams {
                ...
                _padding1: 0,
                            ...
                            derivative_power: 2.0,
                derivative_offset: 1.0,
                transform_coeff_xy: 2.0,
                transform_coeff_xz: 2.0,
                ...
                palette_b_r: 0.5, palette_b_g: 0.3, palette_b_b: 0.1,
                ...
                background_r: 0.05, background_g: 0.05, background_b: 0.1,
                sun_color_r: 6.0, sun_color_g: 4.5, sun_color_b: 3.0,
                fog_color_r: 0.05, fog_color_g: 0.05, fog_color_b: 0.1,
                glow_color_r: 0.3, glow_color_g: 0.5, glow_color_b: 0.8,
            },
        ...
        let shader_source = std::fs::read_to_string("shaders/quadraticbulb.wgsl")
            .unwrap_or_else(|_| include_str!("../../shaders/quadraticbulb.wgsl").to_string());
        
        let cs_module = core.device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("QuadraticBulb Compute Shader"),
            source: wgpu::ShaderSource::Wgsl(shader_source.into()),
        });
        
        let hot_reload = cuneus::ShaderHotReload::new_compute(
            core.device.clone(),
            PathBuf::from("shaders/quadraticbulb.wgsl"),
            cs_module.clone(),
            "main",
        ...
        let compute_pipeline_layout = core.device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("QuadraticBulb Compute Pipeline Layout"),
            bind_group_layouts: &[
                &time_bind_group_layout,
                &params_bind_group_layout,
        ...
        let compute_pipeline = core.device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("QuadraticBulb Compute Pipeline"),
    ...
    fn update(&mut self, core: &Core) {
        if let Some(new_shader) = self.hot_reload.reload_compute_shader() {
            println!("Reloading QuadraticBulb shader at time: {:.2}s", self.base.start_time.elapsed().as_secs_f32());
            
            let compute_pipeline_layout = core.device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("Updated QuadraticBulb Compute Pipeline Layout"),
                bind_group_layouts: &[
            ...
            self.compute_pipeline = core.device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                label: Some("Updated QuadraticBulb Pipeline"),
                layout: Some(&compute_pipeline_layout),
                module: &new_shader,
                entry_point: Some("main"),
                compilation_options: wgpu::PipelineCompilationOptions::default(),
    ...
    fn render(&mut self, core: &Core) -> Result<(), wgpu::SurfaceError> {
        ...
        let full_output = if self.base.key_handler.show_ui {
            self.base.render_ui(core, |ctx| {
                ...
                egui::Window::new("QuadraticBulb PathTracer")
                    .collapsible(true)
                    .resizable(true)
                    .default_width(350.0)
                    .show(ctx, |ui| {
                        ...
                        egui::CollapsingHeader::new("Mathematical Formula")
                            ...
                            .show(ui, |ui| {
                                ...
                                ui.label("Derivative: dr = power * r * dr + offset");
                                let old_deriv_power = params.derivative_power;
                                changed |= ui.add(egui::Slider::new(&mut params.derivative_power, 0.5..=5.0).text("Derivative Power")).changed();
                                if params.derivative_power != old_deriv_power {
                                    self.should_reset_accumulation = true;
                                }

                                let old_deriv_offset = params.derivative_offset;
                                changed |= ui.add(egui::Slider::new(&mut params.derivative_offset, 0.0..=3.0).text("Derivative Offset")).changed();
                                if params.derivative_offset != old_deriv_offset {
                                    self.should_reset_accumulation = true;
                                }

                                ui.separator();
                                ui.label("Transform: z = (x²-y²-z², coeff_xy*xy, coeff_xz*xz)");
                                
                                let old_xy = params.transform_coeff_xy;
                                changed |= ui.add(egui::Slider::new(&mut params.transform_coeff_xy, -5.0..=5.0).text("XY Coefficient")).changed();
                                if params.transform_coeff_xy != old_xy {
                                    self.should_reset_accumulation = true;
                                }

                                let old_xz = params.transform_coeff_xz;
                                changed |= ui.add(egui::Slider::new(&mut params.transform_coeff_xz, -5.0..=5.0).text("XZ Coefficient")).changed();
                                if params.transform_coeff_xz != old_xz {
                                    self.should_reset_accumulation = true;
                                }

                                if ui.button("Reset to Quadratic (2.0, 1.0, 2.0, 2.0)").clicked() {
                                    params.derivative_power = 2.0;
                                    params.derivative_offset = 1.0;
                                    params.transform_coeff_xy = 2.0;
                                    params.transform_coeff_xz = 2.0;
                                    self.should_reset_accumulation = true;
                                    changed = true;
                                }
                            });
        ...
        
        {
            let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("QuadraticBulb Compute Pass"),
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    env_logger::init();
    let (app, event_loop) = cuneus::ShaderApp::new("QuadraticBulb", 800, 600);
    
    app.run(event_loop, |core| {
        QuadraticBulbShader::init(core)
    })
}