...
impl ShaderManager for TreeShader {
    ...
    fn update(&mut self, core: &Core) {
        if let Some(new_shader) = self.multi_buffer.hot_reload.reload_compute_shader() {
            ...
            let time_layout = create_bind_group_layout(&core.device, BindGroupLayoutType::TimeUniform, "Tree Time");
            let params_layout = create_bind_group_layout(&core.device, BindGroupLayoutType::CustomUniform, "Tree Params");
