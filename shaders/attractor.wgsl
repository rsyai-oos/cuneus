...
fn fs_pass1(@builtin(position) FragCoord: vec4<f32>, @location(0) tex_coords: vec2<f32>) -> @location(0) vec4<f32> {
    // Get actual dimensions for proper aspect ratio
...
fn fs_pass2(@builtin(position) FragCoord: vec4<f32>, @location(0) tex_coords: vec2<f32>) -> @location(0) vec4<f32> {
    let prevFrame = textureSample(prev_frame, tex_sampler, tex_coords);
    let currentFrame = textureSample(prev_frame, tex_sampler, tex_coords);
    return mix(prevFrame * params.decay, currentFrame, 0.1);
}
