...
fn droste(z: vec2<f32>, zoom: f32, twists: f32, scale: f32) -> vec2<f32> {
    let ratio = 1.0 / scale;
    let angle = atan(log(ratio) * twists / TWO_PI);
    
    var result = complex_exp(
        complex_div(
            complex_log(z), 
...
fn fs_main(@builtin(position) FragCoord: vec4<f32>, @location(0) tex_coords: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    uv += vec2<f32>(params.offset_x, params.offset_y);
    
    var current_zoom = params.zoom;
    if (params.use_animation > 0.5) {
        let ft = fract(u_time.time * params.time_scale);
        current_zoom = 1.0 + ft * (params.scale - 1.0);
    }
    
    uv = droste(uv, current_zoom, params.branches, params.scale);
